...
import { FileText, Ruler, Layers, Grid3X3, ArrowUpDown, Building2, Download, GitBranch, AlertTriangle, XCircle, Eye, EyeOff, FolderOpen } from "lucide-react";
...
export function Step3CAD({ project, stepStatus, stepNumber, projectId, onProcess, onApprove, onRedo, onGoBack, backLabel, isProcessing, isApproving }: Props) {
  ...
  const hasTelegramResult = hasDrawings || !!(cadResult as any)?.completed;
  const isCompleted = !!(cadResult as any)?.completed && hasDrawings && localProgress >= 100;
  ...
  useEffect(() => {
    const startedAt = (cadResult as any)?.startedAt || Date.now();
    const minDisplayMs = (cadResult as any)?.minDisplayMs || 600000;

    const tick = () => {
      const elapsed = Date.now() - startedAt;
      if (hasTelegramResult) {
        setLocalProgress((prev) => (prev >= 100 ? 100 : Math.min(100, prev + 5)));
    ...
    tick();
    const timer = setInterval(tick, hasTelegramResult ? 150 : 1000);
    return () => clearInterval(timer);
  }, [cadResult, hasTelegramResult]);
            ...
            <div>
              ...
              <div className="space-y-4">
                {cadResult.cadDrawings?.map((d, i) => {
                  // Find matching SVG for technical view
                  ...
                  return (
                    <div key={i} className="rounded-xl overflow-hidden border-2 border-slate-200 shadow-sm" data-testid={`cad-drawing-${i}`}>
                      ...
                      <div className="bg-white p-2 overflow-hidden">
                        {/* Lớp Kỹ thuật — SVG thuần (technical view) */}
                        {false && matchSvg?.svgUrl && (
                          <div style={{ position: "relative", width: "100%", overflow: "hidden" }}>
                            ...
                            />
                            {/* Lớp 3: Text labels — room names + area (technical view) */}
                            {(() => {
                              const matchRoomData = cadResult.roomData?.find(r => r.floor === d.floor);
                                ...
                                <div style={{ position: "absolute", top: 0, left: 0, width: "100%", height: "100%", pointerEvents: "none" }}>
                                  {matchRoomData.rooms.map((room, ri) => (
                                    <div
                                      ...
                                      style={{
                                        position: "absolute",
                                        left: `${room.cxRatio * 100}%`,
                                        top: `${room.cyRatio * 100}%`,
                                    ...
                                    >
                                      <div style={{ fontSize: "clamp(6px, 1.2vw, 10px)", fontWeight: 700, color: "#111", fontFamily: "Arial Narrow, Arial, sans-serif", whiteSpace: "nowrap", textShadow: "0 0 3px white, 0 0 3px white" }}>
                                        {room.name}
                                      </div>
                                      <div style={{ fontSize: "clamp(5px, 1vw, 8px)", color: "#333", fontFamily: "Arial, sans-serif", whiteSpace: "nowrap", textShadow: "0 0 2px white" }}>
                                        {room.area}m²
                                      </div>
                                    </div>
                                  ))}
                        ...
                        {/* Lớp AI Render — AI-generated SVG (vectorized from gpt-image-1 PNG via potrace) */}
                        ...
                        {/* Kết hợp — Lớp 1: CAD SVG (nền), Lớp 2: AI Render SVG (chồng, multiply) */}
                        {false && matchSvg?.svgUrl && (
                          <div style={{ position: "relative", width: "100%", overflow: "hidden" }}>
                            {/* Lớp 1: CAD kỹ thuật — nền chính xác */}
                            <img
                              src={matchSvg.svgUrl}
                              alt={`${d.name} — Kỹ thuật`}
                              style={{ width: "100%", maxWidth: "100%", display: "block", maxHeight: "700px", objectFit: "contain" }}
                            />
                            {/* Lớp 2: AI Render — chồng lên, blend multiply để giữ nét CAD */}
                            {d.imageUrl && (
                              <img
                                src={d.imageUrl}
                                alt={`${d.name} — AI`}
                            ...
                            {/* Lớp 3: Text labels — room names + area, positioned via ratio coords */}
                            {(() => {
                              const matchRoomData = cadResult.roomData?.find(r => r.floor === d.floor);
                              if (!matchRoomData?.rooms?.length) return null;
                              return (
    ...
    >
      {!isCompleted ? (
        <div className="bg-slate-50 rounded-xl p-4">
          <ProgressCircle progress={localProgress} message={timeoutError ? "AutoCAD đang có lỗi, vui lòng thử lại" : ""} />
        </div>
      ) : (
